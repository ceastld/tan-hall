...
(function () {
  ...
  const MAP_NAME = { plain: '平原', canyon: '峡谷', twin: '双台', spire: '风柱', bridge: '碎桥', isles: '悬岛', ruins: '残垣', vale: '风谷', forge: '熔台', arcade: '廊桥', towers: '双塔', moon: '月池', cliff: '断崖', dune: '沙脊', gate: '石门', frost: '霜泽', cloud: '云台', mirror: '镜廊', well: '井口', cave: '洞顶', teeth: '齿岸', ring: '环坑', hour: '砂漏', ridge: '折脊' };
  const MAP_IDS = ['plain', 'canyon', 'twin', 'spire', 'bridge', 'isles', 'ruins', 'vale', 'forge', 'arcade', 'towers', 'moon', 'cliff', 'dune', 'gate', 'frost', 'cloud', 'mirror', 'well', 'cave', 'teeth', 'ring', 'hour', 'ridge'];
  const WALL_MAXH = 160;
  const FIRE_R = 28;
  const FIRE_DMG = 8;
  const FIRE_LIFE = 2;
  ...
  const RING_F2X = 736;
  const RING_CX = 480;
  const RING_BANK_Y = 308;
  const RING_RIM_Y = 250;
  const RING_BOWL_Y = 390;
  const RING_INNER = 140;
  const RING_RIM_W = 36;
  const RING_OUTER = RING_INNER + RING_RIM_W;
  const RING_RIM0 = RING_CX - RING_OUTER;
  const RING_RIM1 = RING_CX - RING_INNER;
  const RING_RIM2 = RING_CX + RING_INNER;
  const RING_RIM3 = RING_CX + RING_OUTER;
  const RING_L1 = RING_RIM0;
  const RING_R0 = RING_RIM3;
  const RING_LIP_L = (RING_RIM0 + RING_RIM1) >> 1;
  const RING_LIP_R = (RING_RIM2 + RING_RIM3) >> 1;
  const RING_WALK = 0.55;
  ...
  const HOUR_F2X = 736;
  const HOUR_CX = 480;
  const HOUR_BANK_Y = 308;
  const HOUR_SHOULDER_Y = 230;
  const HOUR_WAIST_Y = 400;
  const HOUR_WAIST_W = 90;
  const HOUR_WAIST_HW = 45;
  const HOUR_L_SH = 360;
  const HOUR_R_SH = 600;
  const HOUR_W0 = HOUR_CX - HOUR_WAIST_HW;
  const HOUR_W1 = HOUR_CX + HOUR_WAIST_HW;
  const HOUR_BANK_L = 220;
  const HOUR_BANK_R = 740;
  const HOUR_L1 = 248;
  const HOUR_R0 = 712;
  const HOUR_LIP_L = 378;
  const HOUR_LIP_R = 582;
  const HOUR_SH_HW = 72;
  const HOUR_WALK = 0.85;
  const HOUR_CRATER = 0.75;
  const HOUR_STORM_P = 0.30;
  const HOUR_NAME = '砂漏';
  const RIDGE_PX = 168;
  const RIDGE_FX = 772;
  const RIDGE_P2X = 204;
  const RIDGE_F2X = 736;
  const RIDGE_CX = 480;
  const RIDGE_BANK_Y = 308;
  const RIDGE_PEAK_Y = 200;
  const RIDGE_STONE_Y = 236;
  const RIDGE_FLANK_Y = 368;
  const RIDGE_BASE_W = 120;
  const RIDGE_BASE_HW = 60;
  const RIDGE_STONE0 = RIDGE_CX - RIDGE_BASE_HW;
  const RIDGE_STONE1 = RIDGE_CX + RIDGE_BASE_HW;
  const RIDGE_PIT_D = 150;
  const RIDGE_BANK_D = 260;
  const RIDGE_BANK_L = RIDGE_CX - RIDGE_BANK_D;
  const RIDGE_BANK_R = RIDGE_CX + RIDGE_BANK_D;
  const RIDGE_L1 = 248;
  const RIDGE_R0 = 712;
  const RIDGE_LIP_L = 216;
  const RIDGE_LIP_R = 744;
  const RIDGE_WALK = 0.92;
  const RIDGE_CRATER = 0.70;
  const RIDGE_STORM_P = 0.30;
  const RIDGE_NAME = '折脊';
  const DUST_NAME = '坑尘';
  const DUST_IDLE_N = 5;
  ...
  function ringBowlY(x) {
    const d = Math.abs(x - RING_CX);
    if (d >= RING_INNER) return RING_RIM_Y;
    const t = d / RING_INNER;
    const k = 0.5 * (1 + Math.cos(Math.PI * t));
    return lerp(RING_RIM_Y, RING_BOWL_Y, k);
  }

  function ringYAt(x) {
    const d = Math.abs(x - RING_CX);
    if (d <= RING_INNER) return ringBowlY(x);
    if (d <= RING_OUTER) return RING_RIM_Y;
    const bankD = RING_CX - 220;
    if (d >= bankD) return RING_BANK_Y;
    const u = (d - RING_OUTER) / Math.max(1, bankD - RING_OUTER);
    const sm = u * u * (3 - 2 * u);
    return lerp(RING_RIM_Y, RING_BANK_Y, sm);
  }

  function isRingRimX(x) {
    if (G.mapId !== 'ring' || !G.H) return false;
    const i = x | 0;
    if (i < 0 || i >= VW) return false;
    const d = Math.abs(i - RING_CX);
    if (d < RING_INNER - 2 || d > RING_OUTER + 16) return false;
    return G.H[i] < RING_RIM_Y + 32 && G.H[i] < RING_BANK_Y - 12;
  }

  function isRingBowlX(x) {
    if (G.mapId !== 'ring' || !G.H) return false;
    const i = x | 0;
    if (i < RING_RIM1 - 2 || i > RING_RIM2 + 2) return false;
    if (isRingRimX(i)) return false;
    return G.H[i] > RING_RIM_Y + 24;
  }

  function isRingBank(x) {
    if (G.mapId !== 'ring' || !G.H) return false;
    const i = x | 0;
    if (i < 0 || i >= VW) return false;
    if (isDeathVoid(i)) return false;
    if (isRingRimX(i) || isRingBowlX(i)) return false;
    return G.H[i] < RING_BANK_Y + 36;
  }

  function isRingLipX(x) {
    if (G.mapId !== 'ring' || !G.H) return false;
    const i = x | 0;
    if (!isRingRimX(i) && !isRingBank(i) && !isRingBowlX(i)) return false;
    return Math.abs(i - RING_LIP_L) <= 22 || Math.abs(i - RING_LIP_R) <= 22;
  }

  function inRingBowl(u) {
    if (!u || G.mapId !== 'ring') return false;
    return isRingBowlX(u.x);
  }

  function ringFarShore(from, foe) {
    if (!from || !foe) return false;
    if (inRingBowl(from) || inRingBowl(foe)) return false;
    if (!isRingBank(from.x) || !isRingBank(foe.x)) return false;
    return (from.x < RING_CX) !== (foe.x < RING_CX);
  }

  function ringLipX(foe) {
    if (!foe) return RING_LIP_R;
    return foe.x < RING_CX ? RING_LIP_L : RING_LIP_R;
  }
  ...
  function hourSm(a, b, x) {
    const k = clamp((x - a) / Math.max(1, b - a), 0, 1);
    return k * k * (3 - 2 * k);
  }

  function hourYAt(x) {
    if (x <= HOUR_BANK_L) return HOUR_BANK_Y;
    if (x >= HOUR_BANK_R) return HOUR_BANK_Y;
    if (x < HOUR_L_SH) return lerp(HOUR_BANK_Y, HOUR_SHOULDER_Y, hourSm(HOUR_BANK_L, HOUR_L_SH, x));
    if (x < HOUR_W0) return lerp(HOUR_SHOULDER_Y, HOUR_WAIST_Y, hourSm(HOUR_L_SH, HOUR_W0, x));
    if (x <= HOUR_W1) return HOUR_WAIST_Y;
    if (x <= HOUR_R_SH) return lerp(HOUR_WAIST_Y, HOUR_SHOULDER_Y, hourSm(HOUR_W1, HOUR_R_SH, x));
    return lerp(HOUR_SHOULDER_Y, HOUR_BANK_Y, hourSm(HOUR_R_SH, HOUR_BANK_R, x));
  }

  function isHourShoulderX(x) {
    if (G.mapId !== 'hour' || !G.H) return false;
    const i = x | 0;
    if (i < 0 || i >= VW) return false;
    const d = Math.min(Math.abs(i - HOUR_L_SH), Math.abs(i - HOUR_R_SH));
    if (d > HOUR_SH_HW + 2) return false;
    return G.H[i] < (HOUR_SHOULDER_Y + HOUR_BANK_Y) * 0.5;
  }

  function isHourWaistX(x) {
    if (G.mapId !== 'hour' || !G.H) return false;
    const i = x | 0;
    if (i < HOUR_W0 - 2 || i > HOUR_W1 + 2) return false;
    if (isHourShoulderX(i)) return false;
    return G.H[i] > HOUR_BANK_Y + 16;
  }

  function isHourBank(x) {
    if (G.mapId !== 'hour' || !G.H) return false;
    const i = x | 0;
    if (i < 0 || i >= VW) return false;
    if (isDeathVoid(i)) return false;
    if (i > HOUR_L1 && i < HOUR_R0) return false;
    if (isHourShoulderX(i) || isHourWaistX(i)) return false;
    return G.H[i] < HOUR_BANK_Y + 36;
  }

  function isHourLipX(x) {
    if (G.mapId !== 'hour' || !G.H) return false;
    const i = x | 0;
    if (!isHourShoulderX(i) && !isHourBank(i) && !isHourWaistX(i)) return false;
    return Math.abs(i - HOUR_LIP_L) <= 22 || Math.abs(i - HOUR_LIP_R) <= 22;
  }

  function inHourWaist(u) {
    if (!u || G.mapId !== 'hour') return false;
    return isHourWaistX(u.x);
  }

  function hourFarShore(from, foe) {
    if (!from || !foe) return false;
    if (inHourWaist(from) || inHourWaist(foe)) return false;
    if (!isHourBank(from.x) || !isHourBank(foe.x)) return false;
    return (from.x < HOUR_CX) !== (foe.x < HOUR_CX);
  }

  function hourLipX(foe) {
    if (!foe) return HOUR_LIP_R;
    return foe.x < HOUR_CX ? HOUR_LIP_L : HOUR_LIP_R;
  }

  function hourR(r, x) {
    if (G.mapId !== 'hour' || r <= 0) return r;
    if (isHourShoulderX(x)) return r * HOUR_CRATER;
    return r;
  }

  function ridgeSm(a, b, x) {
    const k = clamp((x - a) / Math.max(1, b - a), 0, 1);
    return k * k * (3 - 2 * k);
  }

  function ridgeYAt(x) {
    const d = Math.abs(x - RIDGE_CX);
    if (d >= RIDGE_BANK_D) return RIDGE_BANK_Y;
    if (d <= RIDGE_BASE_HW) {
      const t = d / RIDGE_BASE_HW;
      const k = Math.pow(Math.max(0, Math.cos(t * Math.PI * 0.5)), 1.25);
      return lerp(RIDGE_STONE_Y, RIDGE_PEAK_Y, k);
    }
    if (d <= RIDGE_PIT_D) return lerp(RIDGE_STONE_Y, RIDGE_FLANK_Y, ridgeSm(RIDGE_BASE_HW, RIDGE_PIT_D, d));
    return lerp(RIDGE_FLANK_Y, RIDGE_BANK_Y, ridgeSm(RIDGE_PIT_D, RIDGE_BANK_D, d));
  }

  function isRidgeCrestX(x) {
    if (G.mapId !== 'ridge' || !G.H) return false;
    const i = x | 0;
    if (i < 0 || i >= VW) return false;
    const d = Math.abs(i - RIDGE_CX);
    if (d > RIDGE_BASE_HW + 2) return false;
    return G.H[i] < (RIDGE_PEAK_Y + RIDGE_BANK_Y) * 0.5;
  }

  function isRidgeFlankX(x) {
    if (G.mapId !== 'ridge' || !G.H) return false;
    const i = x | 0;
    if (i <= RIDGE_L1 || i >= RIDGE_R0) return false;
    if (isRidgeCrestX(i)) return false;
    return true;
  }

  function isRidgeBank(x) {
    if (G.mapId !== 'ridge' || !G.H) return false;
    const i = x | 0;
    if (i < 0 || i >= VW) return false;
    if (isDeathVoid(i)) return false;
    if (isRidgeCrestX(i) || isRidgeFlankX(i)) return false;
    return G.H[i] < RIDGE_BANK_Y + 36;
  }

  function isRidgeLipX(x) {
    if (G.mapId !== 'ridge' || !G.H) return false;
    const i = x | 0;
    if (!isRidgeBank(i) && !isRidgeFlankX(i) && !isRidgeCrestX(i)) return false;
    return Math.abs(i - RIDGE_LIP_L) <= 22 || Math.abs(i - RIDGE_LIP_R) <= 22;
  }

  function inRidgeFlank(u) {
    if (!u || G.mapId !== 'ridge') return false;
    return isRidgeFlankX(u.x);
  }

  function ridgeFarShore(from, foe) {
    if (!from || !foe) return false;
    if (inRidgeFlank(from) || inRidgeFlank(foe)) return false;
    if (!isRidgeBank(from.x) || !isRidgeBank(foe.x)) return false;
    return (from.x < RIDGE_CX) !== (foe.x < RIDGE_CX);
  }

  function ridgeLipX(foe) {
    if (!foe) return RIDGE_LIP_R;
    return foe.x < RIDGE_CX ? RIDGE_LIP_L : RIDGE_LIP_R;
  }

  function ridgeR(r, x) {
    if (G.mapId !== 'ridge' || r <= 0) return r;
    if (isRidgeCrestX(x)) return r * RIDGE_CRATER;
    return r;
  }
  ...
  function onGrass(u) {
    ...
    if (G.mapId === 'ring' && (isRingRimX(u.x) || isRingBowlX(u.x))) return false;
    if (G.mapId === 'hour' && (isHourShoulderX(u.x) || isHourWaistX(u.x))) return false;
    if (G.mapId === 'ridge' && (isRidgeCrestX(u.x) || isRidgeFlankX(u.x))) return false;
  ...
  function crateGroundOk(x) {
    ...
    if (G.mapId === 'ring' && (isRingRimX(x) || isRingBowlX(x) || !isRingBank(x))) return false;
    if (G.mapId === 'hour' && (isHourShoulderX(x) || isHourWaistX(x) || !isHourBank(x))) return false;
    if (G.mapId === 'ridge' && (isRidgeCrestX(x) || isRidgeFlankX(x) || !isRidgeBank(x))) return false;
  ...
  function pickStorm(id) {
    ...
    if (id === 'ring') return Math.random() < RING_STORM_P;
    if (id === 'hour') return Math.random() < HOUR_STORM_P;
    if (id === 'ridge') return Math.random() < RIDGE_STORM_P;
  ...
  function walkSpd(u) {
    ...
    else if (inRingBowl(u)) spd = base * RING_WALK;
    else if (inHourWaist(u)) spd = base * HOUR_WALK;
    else if (inRidgeFlank(u)) spd = base * RIDGE_WALK;
  ...
  function buildHeight(id) {
    ...
    if (id === 'canyon') {
      ...
      for (let x = 0; x < VW; x++) {
        let yy = ringYAt(x);
        yy += Math.sin(x * 0.07) * 1.0 + Math.sin(x * 0.19) * 0.4;
        h[x] = yy;
      }
      padFlat(h, 72, 220, RING_BANK_Y);
      padFlat(h, 740, 888, RING_BANK_Y);
    } else if (id === 'hour') {
      for (let x = 0; x < VW; x++) {
        let yy = hourYAt(x);
        yy += Math.sin(x * 0.07) * 1.0 + Math.sin(x * 0.19) * 0.4;
      ...
      padFlat(h, 72, 220, HOUR_BANK_Y);
      padFlat(h, 740, 888, HOUR_BANK_Y);
    } else if (id === 'ridge') {
      for (let x = 0; x < VW; x++) {
        let yy = ridgeYAt(x);
        yy += Math.sin(x * 0.07) * 1.0 + Math.sin(x * 0.19) * 0.4;
      ...
      padFlat(h, 72, 220, RIDGE_BANK_Y);
      padFlat(h, 740, 888, RIDGE_BANK_Y);
    } else {
      for (let x = 0; x < VW; x++) {
        const t = x / (VW - 1);
  ...
  function spawnX(id, side) {
    ...
    if (id === 'ring') return side === 'p' ? RING_PX : RING_FX;
    if (id === 'hour') return side === 'p' ? HOUR_PX : HOUR_FX;
    if (id === 'ridge') return side === 'p' ? RIDGE_PX : RIDGE_FX;
  ...
  function spawnAt(side, slot) {
    ...
    if (G.mapId === 'ring' && slot) return side === 'p' ? RING_P2X : RING_F2X;
    if (G.mapId === 'hour' && slot) return side === 'p' ? HOUR_P2X : HOUR_F2X;
    if (G.mapId === 'ridge' && slot) return side === 'p' ? RIDGE_P2X : RIDGE_F2X;
  ...
  function explode(x, y, wep, owner, fromHit, shot, opts) {
    ...
    crater = Math.round(ringR(crater, x));
  ...
  function denyZoneDist(foe) {
    ...
    if (G.mapId === 'ring') {
      if (inRingBowl(foe)) return 1e9;
      const d = Math.abs(foe.x - ringLipX(foe));
      return d < 8 ? 16 : d;
    }
    if (G.mapId === 'hour') {
      if (inHourWaist(foe)) return 1e9;
      const d = Math.abs(foe.x - hourLipX(foe));
      return d < 8 ? 16 : d;
    }
    if (G.mapId === 'ridge') {
      if (inRidgeFlank(foe)) return 1e9;
      const d = Math.abs(foe.x - ridgeLipX(foe));
      return d < 8 ? 16 : d;
    }
    return 1e9;
  }
  ...
  function denySpotX(from, foe) {
    ...
    if (G.mapId === 'ring') return ringLipX(foe);
    if (G.mapId === 'hour') return hourLipX(foe);
    if (G.mapId === 'ridge') return ridgeLipX(foe);
    return 0;
  }
  ...
  function wantChiLei(from, foe) {
    ...
    if (G.mapId !== 'gate' && G.mapId !== 'dune' && G.mapId !== 'frost' && G.mapId !== 'well' && G.mapId !== 'cave' && G.mapId !== 'teeth' && G.mapId !== 'ring' && G.mapId !== 'hour' && G.mapId !== 'ridge') return false;
    ...
    if (G.mapId === 'ring') {
      if (inRingBowl(foe)) return false;
      if ((from.x < RING_CX) === (foe.x < RING_CX)) return false;
      if (Math.abs(foe.x - ringLipX(foe)) > 40) return false;
    }
    if (G.mapId === 'hour') {
      if (inHourWaist(foe)) return false;
      if ((from.x < HOUR_CX) === (foe.x < HOUR_CX)) return false;
      if (Math.abs(foe.x - hourLipX(foe)) > 40) return false;
    }
    if (G.mapId === 'ridge') {
      if (inRidgeFlank(foe)) return false;
      if ((from.x < RIDGE_CX) === (foe.x < RIDGE_CX)) return false;
      if (Math.abs(foe.x - ridgeLipX(foe)) > 40) return false;
    }
  ...
  function pickAIWeapon(from) {
    ...
    if (foe) {
      ...
      if (G.mapId === 'ring' && inRingBowl(foe)) return 1;
      if (G.mapId === 'hour' && inHourWaist(foe)) return 1;
      if (G.mapId === 'ridge' && inRidgeFlank(foe)) return 1;
      ...
      if (G.mapId === 'ring' && ringFarShore(from, foe)) return 0;
      if (G.mapId === 'hour' && hourFarShore(from, foe)) return 0;
      if (G.mapId === 'ridge' && ridgeFarShore(from, foe)) return 0;
    }
  ...
  function scoreOne(imp, wep, from, t, ang) {
    ...
    if (G.mapId === 'ring' && ringFarShore(from, t)) {
      const e = ang != null ? elev(ang) : 0;
      if (e >= 28 && e <= 36) score += 1800;
      else if (e >= 22 && e <= 42) score += 700;
      else if (e >= 58) score -= 500;
    }
    if (G.mapId === 'ring' && isRingBowlX(imp.x) && (wep.id === 1 || wep.id === 4)) bury += 700;
    if (G.mapId === 'ring' && isRingLipX(imp.x) && wep.id === 8) bury += 500;
    if (G.mapId === 'hour' && inHourWaist(t)) {
      const e = ang != null ? elev(ang) : 0;
      if (e >= 78) score += 2000;
      else if (e >= 70) score += 900;
      if (wep.id === 1) score += 500;
    ...
    if (G.mapId === 'hour' && hourFarShore(from, t)) {
      const e = ang != null ? elev(ang) : 0;
      if (e >= 28 && e <= 36) score += 1800;
      else if (e >= 22 && e <= 42) score += 700;
      else if (e >= 58) score -= 500;
    }
    if (G.mapId === 'hour' && isHourWaistX(imp.x) && (wep.id === 1 || wep.id === 4)) bury += 700;
    if (G.mapId === 'hour' && isHourLipX(imp.x) && wep.id === 8) bury += 500;
    if (G.mapId === 'ridge' && inRidgeFlank(t)) {
      const e = ang != null ? elev(ang) : 0;
      if (e >= 78) score += 2000;
      else if (e >= 70) score += 900;
      if (wep.id === 1) score += 500;
    ...
    if (G.mapId === 'ridge' && ridgeFarShore(from, t)) {
      const e = ang != null ? elev(ang) : 0;
      if (e >= 28 && e <= 36) score += 1800;
      else if (e >= 22 && e <= 42) score += 700;
      else if (e >= 58) score -= 500;
    }
    if (G.mapId === 'ridge' && isRidgeFlankX(imp.x) && (wep.id === 1 || wep.id === 4)) bury += 700;
    if (G.mapId === 'ridge' && isRidgeLipX(imp.x) && wep.id === 8) bury += 500;
    if (wep.id === 8) {
      ...
      if (spot) {
        ...
        else if (G.mapId === 'ring' && isRingLipX(imp.x)) score += 900;
        else if (G.mapId === 'hour' && isHourLipX(imp.x)) score += 900;
        else if (G.mapId === 'ridge' && isRidgeLipX(imp.x)) score += 900;
      }
    }
    score += bury;
    return score;
  ...
  function paintTerrain() {
    ...
    const H = G.H;
    if (!H) return;
    const top = G.mapId === 'canyon' ? '#5ad6ff' : G.mapId === 'twin' ? '#ffe36b' : G.mapId === 'spire' ? '#9af0ff' : G.mapId === 'bridge' ? '#e8c090' : G.mapId === 'isles' ? '#c8f0ff' : G.mapId === 'ruins' ? '#e0c090' : G.mapId === 'vale' ? '#7cf6ff' : G.mapId === 'forge' ? '#ff8a40' : G.mapId === 'arcade' ? '#e4d2a8' : G.mapId === 'towers' ? '#d8c4a0' : G.mapId === 'moon' ? '#c8eeff' : G.mapId === 'cliff' ? '#ffc078' : G.mapId === 'dune' ? '#f0c878' : G.mapId === 'gate' ? '#d8c8a8' : G.mapId === 'frost' ? '#d4f2ff' : G.mapId === 'cloud' ? '#e8d8b8' : G.mapId === 'mirror' ? '#c8e8ff' : G.mapId === 'well' ? '#c8b898' : G.mapId === 'cave' ? '#d8c4a0' : G.mapId === 'teeth' ? '#e0c8a0' : G.mapId === 'ring' ? '#c8b090' : G.mapId === 'hour' ? '#e0c49a' : G.mapId === 'ridge' ? '#d4c4a8' : '#7dffc6';
    const mid = G.mapId === 'canyon' ? '#2a1a48' : G.mapId === 'twin' ? '#2a1840' : G.mapId === 'spire' ? '#143044' : G.mapId === 'bridge' ? '#2a2018' : G.mapId === 'isles' ? '#182438' : G.mapId === 'ruins' ? '#2a1c18' : G.mapId === 'vale' ? '#142038' : G.mapId === 'forge' ? '#3a140c' : G.mapId === 'arcade' ? '#241c18' : G.mapId === 'towers' ? '#221810' : G.mapId === 'moon' ? '#122436' : G.mapId === 'cliff' ? '#3a2214' : G.mapId === 'dune' ? '#4a3018' : G.mapId === 'gate' ? '#2a2218' : G.mapId === 'frost' ? '#143044' : G.mapId === 'cloud' ? '#241c28' : G.mapId === 'mirror' ? '#182030' : G.mapId === 'well' ? '#141018' : G.mapId === 'cave' ? '#1a1410' : G.mapId === 'teeth' ? '#1c1410' : G.mapId === 'ring' ? '#201610' : G.mapId === 'hour' ? '#24180e' : G.mapId === 'ridge' ? '#1c1410' : '#162436';
    const bot = '#0a0614';
    ...
    if (G.mapId === 'ring') {
      const pit = g.createLinearGradient(0, RING_RIM_Y, 0, VH);
      pit.addColorStop(0, 'rgba(18, 12, 16, 0.10)');
      pit.addColorStop(0.22, 'rgba(28, 18, 16, 0.40)');
      pit.addColorStop(0.6, '#14100c');
      pit.addColorStop(1, '#0a0806');
      g.fillStyle = pit;
      g.fillRect(RING_RIM1, RING_RIM_Y + 4, RING_RIM2 - RING_RIM1, VH - RING_RIM_Y - 4);
      g.fillStyle = 'rgba(90, 64, 44, 0.22)';
      g.fillRect(48, RING_BANK_Y - 4, RING_RIM0 - 64, 8);
      g.fillRect(RING_RIM3 + 16, RING_BANK_Y - 4, 900 - RING_RIM3, 8);
      g.fillStyle = 'rgba(196, 156, 112, 0.22)';
      g.fillRect(RING_RIM0, RING_RIM_Y - 3, RING_RIM_W, 7);
      g.fillRect(RING_RIM2, RING_RIM_Y - 3, RING_RIM_W, 7);
      g.strokeStyle = 'rgba(216, 184, 140, 0.58)';
      g.lineWidth = 1.6;
      g.beginPath();
      g.moveTo(RING_RIM0, H[RING_RIM0]);
      for (let x = RING_RIM0; x <= RING_RIM1; x++) g.lineTo(x, H[x]);
      g.stroke();
      g.beginPath();
      g.moveTo(RING_RIM2, H[RING_RIM2]);
      for (let x = RING_RIM2; x <= RING_RIM3; x++) g.lineTo(x, H[x]);
      g.stroke();
      g.strokeStyle = 'rgba(168, 120, 80, 0.42)';
      g.lineWidth = 1.3;
      g.beginPath();
      g.moveTo(RING_RIM1, H[RING_RIM1]);
      for (let x = RING_RIM1; x <= RING_RIM2; x++) g.lineTo(x, H[x]);
      g.stroke();
    }
    if (G.mapId === 'hour') {
      const pit = g.createLinearGradient(0, HOUR_SHOULDER_Y, 0, VH);
      pit.addColorStop(0, 'rgba(18, 12, 10, 0.08)');
      pit.addColorStop(0.22, 'rgba(28, 18, 12, 0.38)');
      pit.addColorStop(0.6, '#16100c');
      pit.addColorStop(1, '#0a0806');
      g.fillStyle = pit;
      g.fillRect(HOUR_W0, HOUR_BANK_Y + 4, HOUR_W1 - HOUR_W0, VH - HOUR_BANK_Y - 4);
      g.fillStyle = 'rgba(90, 64, 44, 0.22)';
      g.fillRect(48, HOUR_BANK_Y - 4, HOUR_L1 - 64, 8);
      g.fillRect(HOUR_R0 + 16, HOUR_BANK_Y - 4, 900 - HOUR_R0, 8);
      g.fillStyle = 'rgba(196, 156, 112, 0.20)';
      g.fillRect(HOUR_L_SH - HOUR_SH_HW, HOUR_SHOULDER_Y - 3, HOUR_SH_HW * 2, 7);
      g.fillRect(HOUR_R_SH - HOUR_SH_HW, HOUR_SHOULDER_Y - 3, HOUR_SH_HW * 2, 7);
      g.strokeStyle = 'rgba(216, 184, 140, 0.58)';
      g.lineWidth = 1.6;
      g.beginPath();
      g.moveTo(HOUR_L_SH - HOUR_SH_HW, H[HOUR_L_SH - HOUR_SH_HW]);
      for (let x = HOUR_L_SH - HOUR_SH_HW; x <= HOUR_L_SH + HOUR_SH_HW; x++) g.lineTo(x, H[x]);
      g.stroke();
      g.beginPath();
      g.moveTo(HOUR_R_SH - HOUR_SH_HW, H[HOUR_R_SH - HOUR_SH_HW]);
      for (let x = HOUR_R_SH - HOUR_SH_HW; x <= HOUR_R_SH + HOUR_SH_HW; x++) g.lineTo(x, H[x]);
      g.stroke();
      g.strokeStyle = 'rgba(168, 120, 80, 0.42)';
      g.lineWidth = 1.3;
      g.beginPath();
      g.moveTo(HOUR_W0, H[HOUR_W0]);
      for (let x = HOUR_W0; x <= HOUR_W1; x++) g.lineTo(x, H[x]);
      g.stroke();
    }
    if (G.mapId === 'ridge') {
      const pit = g.createLinearGradient(0, RIDGE_PEAK_Y, 0, VH);
      pit.addColorStop(0, 'rgba(18, 14, 12, 0.08)');
      pit.addColorStop(0.22, 'rgba(28, 18, 14, 0.36)');
      pit.addColorStop(0.6, '#16110c');
      pit.addColorStop(1, '#0a0806');
      g.fillStyle = pit;
      g.fillRect(RIDGE_BANK_L, RIDGE_BANK_Y + 4, RIDGE_BANK_R - RIDGE_BANK_L, VH - RIDGE_BANK_Y - 4);
      g.fillStyle = 'rgba(90, 64, 44, 0.22)';
      g.fillRect(48, RIDGE_BANK_Y - 4, RIDGE_L1 - 64, 8);
      g.fillRect(RIDGE_R0 + 16, RIDGE_BANK_Y - 4, 900 - RIDGE_R0, 8);
      g.fillStyle = 'rgba(196, 168, 128, 0.22)';
      g.fillRect(RIDGE_STONE0, RIDGE_PEAK_Y - 3, RIDGE_BASE_W, 7);
      g.strokeStyle = 'rgba(216, 196, 160, 0.62)';
      g.lineWidth = 1.7;
      g.beginPath();
      g.moveTo(RIDGE_STONE0, H[RIDGE_STONE0]);
      for (let x = RIDGE_STONE0; x <= RIDGE_STONE1; x++) g.lineTo(x, H[x]);
      g.stroke();
      g.strokeStyle = 'rgba(168, 120, 80, 0.42)';
      g.lineWidth = 1.3;
      g.beginPath();
      g.moveTo(RIDGE_BANK_L, H[RIDGE_BANK_L]);
      for (let x = RIDGE_BANK_L; x <= RIDGE_STONE0; x++) g.lineTo(x, H[x]);
      g.stroke();
      g.beginPath();
      g.moveTo(RIDGE_STONE1, H[RIDGE_STONE1]);
      for (let x = RIDGE_STONE1; x <= RIDGE_BANK_R; x++) g.lineTo(x, H[x]);
      g.stroke();
    }
    terrainDirty = false;
  }
  ...
  function drawSky(g) {
    ...
    if (G.mapId === 'ring') {
      const bowl = g.createRadialGradient(RING_CX, RING_BOWL_Y, 18, RING_CX, RING_BOWL_Y + 40, 220);
      bowl.addColorStop(0, 'rgba(28, 16, 12, 0.38)');
      bowl.addColorStop(1, 'rgba(28, 16, 12, 0)');
      g.fillStyle = bowl;
      g.fillRect(0, 0, VW, VH);
      const rim = g.createRadialGradient(RING_CX, RING_RIM_Y, 24, RING_CX, RING_RIM_Y + 36, 200);
      rim.addColorStop(0, 'rgba(196, 156, 112, 0.12)');
      rim.addColorStop(1, 'rgba(196, 156, 112, 0)');
      g.fillStyle = rim;
      g.fillRect(0, 0, VW, VH);
    }
  ...
  function selfCheck() {
    ...
    ok('maps eighteen', MAP_IDS.length === 24 && MAP_NAME.spire === '风柱' && MAP_NAME.bridge === '碎桥' && MAP_NAME.isles === '悬岛' && MAP_NAME.ruins === '残垣' && MAP_NAME.vale === '风谷' && MAP_NAME.forge === '熔台' && MAP_NAME.arcade === '廊桥' && MAP_NAME.towers === '双塔' && MAP_NAME.moon === '月池' && MAP_NAME.cliff === '断崖' && MAP_NAME.dune === '沙脊' && MAP_NAME.gate === '石门' && MAP_NAME.frost === '霜泽' && MAP_NAME.cloud === '云台' && MAP_NAME.mirror === '镜廊' && MAP_NAME.well === '井口' && MAP_NAME.cave === '洞顶' && MAP_NAME.teeth === '齿岸' && MAP_NAME.ring === '环坑' && MAP_NAME.hour === '砂漏' && MAP_NAME.ridge === '折脊');
    ...
    ok('随图 pool 24', MAP_IDS.length === 24 && MAP_IDS.every(function (id) { return !!MAP_NAME[id]; }) && MAP_IDS.indexOf('moon') === 11 && MAP_IDS.indexOf('cliff') === 12 && MAP_IDS.indexOf('dune') === 13 && MAP_IDS.indexOf('gate') === 14 && MAP_IDS.indexOf('frost') === 15 && MAP_IDS.indexOf('cloud') === 16 && MAP_IDS.indexOf('mirror') === 17 && MAP_IDS.indexOf('well') === 18 && MAP_IDS.indexOf('cave') === 19 && MAP_IDS.indexOf('teeth') === 20 && MAP_IDS.indexOf('ring') === 21 && MAP_IDS.indexOf('hour') === 22 && MAP_IDS.indexOf('ridge') === 23);
    ...
    ok('maps still 14 after ghost', MAP_IDS.length === 24 && MAP_NAME.moon === '月池' && MAP_NAME.cliff === '断崖' && MAP_NAME.dune === '沙脊');
    ...
    ok('maps still 14 after silk', MAP_IDS.length === 24 && MAP_NAME.cliff === '断崖' && MAP_NAME.moon === '月池' && MAP_NAME.dune === '沙脊');
    ...
    ok('maps still 14 after 叠珠', MAP_IDS.length === 24 && MAP_NAME.cliff === '断崖' && MAP_NAME.dune === '沙脊');
    ...
    ok('maps still 14 after 雷泽', MAP_IDS.length === 24 && MAP_NAME.cliff === '断崖' && MAP_NAME.dune === '沙脊' && MAP_NAME.vale === '风谷');
    ...
    ok('maps 15 with 石门', MAP_IDS.length === 24 && MAP_NAME.cliff === '断崖' && MAP_NAME.dune === '沙脊' && MAP_NAME.gate === '石门');
    ...
    ok('maps 15 after 迟雷', MAP_IDS.length === 24 && MAP_NAME.cliff === '断崖' && MAP_NAME.dune === '沙脊' && MAP_NAME.gate === '石门');
    ...
    ok('maps 16 with 霜泽', MAP_IDS.length === 24 && MAP_NAME.cliff === '断崖' && MAP_NAME.dune === '沙脊' && MAP_NAME.gate === '石门' && MAP_NAME.frost === '霜泽');
    ...
    ok('maps still 16 after 堂匣', MAP_IDS.length === 24 && MAP_NAME.cliff === '断崖' && MAP_NAME.dune === '沙脊' && MAP_NAME.gate === '石门' && MAP_NAME.frost === '霜泽');
    ...
    ok('maps still 16 after 时尽', MAP_IDS.length === 24 && MAP_NAME.cliff === '断崖' && MAP_NAME.dune === '沙脊' && MAP_NAME.gate === '石门' && MAP_NAME.frost === '霜泽');
    ...
    ok('maps 17 with 云台', MAP_IDS.length === 24 && MAP_NAME.frost === '霜泽' && MAP_NAME.cloud === '云台' && MAP_NAME.cliff === '断崖');
    ...
    ok('maps 18 with 镜廊', MAP_IDS.length === 24 && MAP_NAME.cloud === '云台' && MAP_NAME.mirror === '镜廊' && MAP_NAME.cliff === '断崖');
    ...
    ok('maps still 18 after 余震', MAP_IDS.length === 24 && MAP_NAME.mirror === '镜廊' && MAP_NAME.cloud === '云台' && MAP_NAME.cliff === '断崖');
    ...
    ok('maps 19 with 井口', MAP_IDS.length === 24 && MAP_NAME.mirror === '镜廊' && MAP_NAME.well === '井口' && MAP_NAME.cloud === '云台');
    ...
    ok('maps 19 after 堂袋', MAP_IDS.length === 24 && MAP_NAME.well === '井口' && MAP_NAME.mirror === '镜廊' && MAP_NAME.cloud === '云台');
    ...
    ok('maps 19 after v40', MAP_IDS.length === 24 && MAP_NAME.well === '井口');
    ...
    ok('maps 19 after v43', MAP_IDS.length === 24 && MAP_NAME.well === '井口');
    ...
    ok('maps 20 with 洞顶', MAP_IDS.length === 24 && MAP_NAME.well === '井口' && MAP_NAME.cave === '洞顶' && MAP_NAME.mirror === '镜廊');
    ...
    ok('maps 20 after 落顶', MAP_IDS.length === 24 && MAP_NAME.cave === '洞顶' && MAP_NAME.well === '井口');
    ...
    ok('maps 21 with 齿岸', MAP_IDS.length === 24 && MAP_NAME.cave === '洞顶' && MAP_NAME.teeth === '齿岸' && MAP_NAME.well === '井口');
    ...
    ok('袋火 maps 21', MAP_IDS.length === 24 && MAP_NAME.teeth === '齿岸' && MAP_NAME.cave === '洞顶');
    ...
    ok('齿岸 still after ×3 分裂', MAP_NAME.teeth === '齿岸' && MAP_IDS.length === 24);
    ...
    ok('maps 21 still after 金匣袋', MAP_IDS.length === 24 && MAP_NAME.teeth === '齿岸' && MAP_NAME.cave === '洞顶');
    ...
    G.mines = [];
    G.walk = WALK_PX;
    ok('ring spawn', spawnX('ring', 'p') === RING_PX && spawnX('ring', 'f') === RING_FX);
    ok('ring spawn on shores', isRingBank(RING_PX) && isRingBank(RING_FX), Math.round(G.H[RING_PX]) + '/' + Math.round(G.H[RING_FX]));
    ok('ring shores y~308', G.H[RING_PX] > 292 && G.H[RING_PX] < 324 && G.H[RING_FX] > 292 && G.H[RING_FX] < 324, Math.round(G.H[RING_PX]));
    ok('ring rim around 480', isRingRimX(RING_LIP_L) && isRingRimX(RING_LIP_R) && G.H[RING_LIP_L] > 236 && G.H[RING_LIP_L] < 278, Math.round(G.H[RING_LIP_L]));
    ok('ring rim y~250', G.H[RING_RIM0 + 8] > 236 && G.H[RING_RIM0 + 8] < 278 && G.H[RING_RIM3 - 8] > 236 && G.H[RING_RIM3 - 8] < 278, Math.round(G.H[RING_RIM0 + 8]));
    ok('ring bowl floor y~390', isRingBowlX(RING_CX) && G.H[RING_CX] > 370 && G.H[RING_CX] < 410, Math.round(G.H[RING_CX]));
    ok('ring inner ~140', isRingBowlX(RING_CX) && isRingBowlX(RING_CX - 80) && !isRingBowlX(RING_RIM0) && isRingRimX(RING_RIM0 + 8));
    ok('ring rim band ~36', isRingRimX(RING_RIM0 + 4) && isRingRimX(RING_RIM1 - 4) && RING_RIM_W === 36);
    ok('ring no void', !isDeathVoid(RING_PX) && !isDeathVoid(RING_FX) && !isDeathVoid(RING_CX) && !isDeathVoid(RING_LIP_L));
    ok('ring nums', RING_WALK === 0.55 && RING_CRATER === 0.72 && RING_STORM_P === 0.25 && GRAV === 260 && VK === 420);
    G.p = { x: RING_PX, y: G.H[RING_PX] - 14, r: 14, hp: 100, max: 100, side: 'p', id: 'p' };
    G.f = { x: RING_FX, y: G.H[RING_FX] - 14, r: 14, hp: 100, max: 100, side: 'f', id: 'f', ang: 115 };
    G.p2 = null; G.f2 = null;
    const rpx = spawnX('ring', 'p');
    const rpy = G.H[rpx | 0] - UNIT_R;
    const rth30 = 30 * Math.PI / 180;
    const rs30 = traceShot(rpx + Math.cos(rth30) * 18, rpy - 4 - Math.sin(rth30) * 18, 30, 94, 0, WEPS[0], G.H, G.p);
    ok('ring 30 skip far shore', isRingBank(rs30.x) && rs30.x > RING_R0 && !rs30.air, Math.round(rs30.x) + ',' + Math.round(rs30.y));
    const rLip = RING_LIP_L;
    const rley = G.H[rLip | 0] - UNIT_R;
    const r90 = 86;
    const rth90 = r90 * Math.PI / 180;
    const rs90 = traceShot(rLip + Math.cos(rth90) * 18, rley - 4 - Math.sin(rth90) * 18, r90, 95, 0, WEPS[0], G.H, G.p);
    ok('ring 90 dunk bowl', isRingBowlX(rs90.x) && !rs90.air, Math.round(rs90.x) + ' a' + r90);
    const rth65 = 65 * Math.PI / 180;
    const rs65 = traceShot(rpx + Math.cos(rth65) * 18, rpy - 4 - Math.sin(rth65) * 18, 65, 32, 0, WEPS[0], G.H, G.p);
    ok('ring 65 clip near rim', isRingRimX(rs65.x) && rs65.x < RING_CX && !rs65.air, Math.round(rs65.x) + ',' + Math.round(rs65.y));
    const rBowl = { x: RING_CX, y: G.H[RING_CX] - 14, r: 14, hp: 100, max: 100, side: 'p', id: 'p' };
    const rRim = { x: RING_LIP_L, y: G.H[RING_LIP_L] - 14, r: 14, hp: 100, max: 100, side: 'f', id: 'f' };
    const rBank = { x: RING_PX, y: G.H[RING_PX] - 14, r: 14, hp: 100, max: 100, side: 'p', id: 'p' };
    ok('ring bowl walk 0.55', inRingBowl(rBowl) && Math.abs(walkSpd(rBowl) - 90 * RING_WALK) < 0.01, walkSpd(rBowl));
    ok('ring rim walk full', isRingRimX(rRim.x) && Math.abs(walkSpd(rRim) - 78) < 0.01, walkSpd(rRim));
    ok('ring shore walk full', isRingBank(rBank.x) && Math.abs(walkSpd(rBank) - 90) < 0.01, walkSpd(rBank));
    G.storm = true;
    ok('ring bowl no storm extra', inRingBowl(rBowl) && !onGrass(rBowl) && Math.abs(walkSpd(rBowl) - 90 * RING_WALK) < 0.01, walkSpd(rBowl));
    ok('ring shore wet grass', onGrass(rBank) && Math.abs(walkSpd(rBank) - 90 * STORM_WALK) < 0.01, walkSpd(rBank));
    G.storm = false;
    const hpSoak = rBowl.hp;
    tickMoonWater(rBowl);
    tickCliffWater(rBowl);
    tickWellWater(rBowl);
    ok('ring no water soak', rBowl.hp === hpSoak && inMoonWater(rBowl) === false && inWellWater(rBowl) === false, rBowl.hp);
    const ringDirtH = new Float32Array(VW);
    for (let i = 0; i < VW; i++) ringDirtH[i] = 400;
    G.H = ringDirtH;
    G.mapId = 'plain';
    carve(500, 400, 30);
    const ringDirt = G.H[500] - 400;
    G.mapId = 'ring';
    for (let i = 0; i < VW; i++) G.H[i] = 400;
    G.H[RING_LIP_L] = RING_RIM_Y;
    carve(RING_LIP_L, RING_RIM_Y, ringR(30, RING_LIP_L));
    const rimEat = G.H[RING_LIP_L] - RING_RIM_Y;
    ok('ring rim crater ~0.72', rimEat > 4 && rimEat < ringDirt * 0.92 && Math.abs(rimEat / ringDirt - RING_CRATER) < 0.12, Math.round(rimEat) + '/' + Math.round(ringDirt));
    G.H = buildHeight('ring');
    G.mapId = 'ring';
    const ringShoreY = G.H[RING_PX];
    carve(RING_PX, ringShoreY, 30);
    const ringShoreEat = G.H[RING_PX] - ringShoreY;
    ok('ring shore crater normal', Math.abs(ringShoreEat - ringDirt) < 3, Math.round(ringShoreEat) + '/' + Math.round(ringDirt));
    const heRim = Math.round(ringR(WEPS[1].crater, RING_LIP_L));
    ok('ring rim 余震', wantQuake(WEPS[1], heRim) === true && heRim >= QUAKE_R * 0.65, heRim);
    G.H = buildHeight('ring');
    G.mapId = 'ring';
    G.kind = 'hall';
    G.wind = 0;
    G.ai = 1;
    G.p = { x: RING_CX, y: G.H[RING_CX] - 14, r: 14, hp: 100, max: 100, side: 'p', id: 'p' };
    G.f = { x: RING_FX, y: G.H[RING_FX] - 14, r: 14, hp: 100, max: 100, side: 'f', id: 'f', ang: 115 };
    G.p2 = null; G.f2 = null;
    ok('ring AI bowl 高爆', inRingBowl(G.p) && pickAIWeapon(G.f) === 1);
    const rimpV = { x: RING_CX, y: G.H[RING_CX], t: 1, hit: null };
    const rsc90 = scoreOne(rimpV, WEPS[1], G.f, G.p, 88);
    const rsc30v = scoreOne(rimpV, WEPS[1], G.f, G.p, 30);
    ok('ring AI prefer 90 dunk', rsc90 > rsc30v + 400, Math.round(rsc90) + '>' + Math.round(rsc30v));
    G.H = buildHeight('ring');
    G.mapId = 'ring';
    G.p = { x: RING_PX, y: G.H[RING_PX] - 14, r: 14, hp: 100, max: 100, side: 'p', id: 'p' };
    G.f = { x: RING_FX, y: G.H[RING_FX] - 14, r: 14, hp: 100, max: 100, side: 'f', id: 'f', ang: 115 };
    ok('ring AI far shore 普通', ringFarShore(G.f, G.p) && pickAIWeapon(G.f) === 0);
    const rimpFar = { x: RING_PX, y: G.H[RING_PX], t: 1, hit: null };
    const rscSkip = scoreOne(rimpFar, WEPS[0], G.f, G.p, 30);
    const rscHi = scoreOne(rimpFar, WEPS[0], G.f, G.p, 65);
    ok('ring AI prefer 30 skip', rscSkip > rscHi + 400, Math.round(rscSkip) + '>' + Math.round(rscHi));
    G.p = { x: RING_LIP_L, y: G.H[RING_LIP_L] - 14, r: 14, hp: 50, max: 100, side: 'p', id: 'p' };
    G.turns = 3;
    ok('ring AI far lip 迟雷', (G.f.x < RING_CX) !== (G.p.x < RING_CX) && isRingLipX(G.p.x) && wantChiLei(G.f, G.p) === true && pickAIWeapon(G.f) === 7);
    G.kind = 'duo';
    const rdx0 = spawnAt('p', 0);
    const rdx1 = spawnAt('p', 1);
    const rdr0 = spawnAt('f', 0);
    const rdr1 = spawnAt('f', 1);
    ok('ring duo extras shore', isRingBank(rdx0) && isRingBank(rdx1) && isRingBank(rdr0) && isRingBank(rdr1), rdx1 + '/' + rdr1);
    ok('ring duo tops', rdx0 === RING_PX && rdr0 === RING_FX);
    ok('ring duo extras along', Math.abs(rdx1 - rdx0) >= 20 && Math.abs(rdr1 - rdr0) >= 20, Math.round(Math.abs(rdx1 - rdx0)));
    ok('ring duo not rim', !isRingRimX(rdx0) && !isRingRimX(rdx1) && !isRingRimX(rdr0) && !isRingRimX(rdr1));
    ok('ring duo not bowl', !isRingBowlX(rdx1) && !isRingBowlX(rdr1));
    ok('ring duo not void', !isDeathVoid(rdx0) && !isDeathVoid(rdx1) && !isDeathVoid(rdr0) && !isDeathVoid(rdr1));
    G.kind = 'hall';
    ok('crate on ring shore', crateGroundOk(RING_PX) === true && crateGroundOk(RING_CX) === false && crateGroundOk(RING_LIP_L) === false);
    ok('ring storm ~25', !stormForced('ring') && !stormBanned('ring') && RING_STORM_P === 0.25 && STORM_P === 0.35);
    ok('环坑 name locked', MAP_NAME.ring === '环坑' && RING_NAME === '环坑' && MAP_IDS[21] === 'ring');
    ok('no banned ring', MAP_NAME.ring.indexOf('传送') < 0 && MAP_NAME.ring.indexOf('飞行') < 0 && MAP_NAME.ring.indexOf('三叉戟') < 0 && MAP_NAME.ring.indexOf('激怒') < 0 && MAP_NAME.ring.indexOf('天使') < 0 && MAP_NAME.ring.indexOf('恶魔') < 0 && MAP_NAME.ring.indexOf('月池') < 0 && MAP_NAME.ring.indexOf('井口') < 0);
    ok('maps 22 with 环坑', MAP_IDS.length === 24 && MAP_NAME.ring === '环坑' && MAP_NAME.teeth === '齿岸' && MAP_NAME.cave === '洞顶' && MAP_NAME.well === '井口' && MAP_NAME.moon === '月池');
    ok('齿岸 still after 环坑', MAP_NAME.teeth === '齿岸' && MAP_IDS[20] === 'teeth');
    ok('洞顶 still after 环坑', MAP_NAME.cave === '洞顶' && MAP_IDS[19] === 'cave');
    ok('井口 still after 环坑', MAP_NAME.well === '井口' && MAP_IDS[18] === 'well');
    ok('月池 still after 环坑', MAP_NAME.moon === '月池');
    ok('叠珠 still 7 after 环坑', WEPS[6] && WEPS[6].name === '叠珠' && WEPS[6].id === 7);
    ok('迟雷 still 8 after 环坑', WEPS[7] && WEPS[7].name === '迟雷' && WEPS[7].id === 8);
    ok('落顶 still after 环坑', FALL_NAME === '落顶' && wantFallChips() === false);
    ok('金匣袋 still after 环坑', GOLD_BAG_NAME === '金匣袋' && GOLD_BAG_P === 0.40 && maybeBagCrate('gold') === 'gold' && BAG_CRATE_P === 0.50);
    ok('×3 fork still after 环坑', bagForkAngles(65).join(',') === '57,65,73' && bagExtraCount(x3u) === 0);
    fireBagU({ x3: true, ang: 65, power: 70, wep: 0 });
    ok('×3 3 shells still after 环坑', G.shots && G.shots.length === 3 && (!G.queue || G.queue.length === 0));
    fireBagU({ x2: true, ang: 65, power: 70, wep: 0, stam: 0 });
    ok('0 stam still after 环坑', G.shots && G.shots.length === 1 && (!G.queue || G.queue.length === 0));
    ok('袋火 still after 环坑', BAG_FIRE_NAME === '袋火' && BAG_TINT.x3 === '#dc143c');
    ok('堂袋 hud still after 环坑', bagStackReadout(x2p5) === '2发×1.35' && bagStackReadout(x3u) === '3发×0.60');
    ok('余震 still after 环坑', QUAKE_NAME === '余震' && HIT_STOP_DIRECT === 0.14);
    ok('雷泽 still after 环坑', STORM_NAME === '雷泽' && stormForced('vale') && stormBanned('forge'));
    ok('ghost K still after 环坑', G.ghostOn !== false && OPS.indexOf('K 残影') >= 0);
    ...
    ok('maps 22 after 蓄条', MAP_IDS.length === 24 && MAP_NAME.ring === '环坑');
    ...
    ok('maps 22 after 坑尘', MAP_IDS.length === 24 && MAP_NAME.ring === '环坑' && MAP_IDS[21] === 'ring');
    ...
    G.mines = [];
    G.walk = WALK_PX;
    ok('hour spawn', spawnX('hour', 'p') === HOUR_PX && spawnX('hour', 'f') === HOUR_FX);
    ok('hour spawn on shores', isHourBank(HOUR_PX) && isHourBank(HOUR_FX), Math.round(G.H[HOUR_PX]) + '/' + Math.round(G.H[HOUR_FX]));
    ok('hour shores y~308', G.H[HOUR_PX] > 292 && G.H[HOUR_PX] < 324 && G.H[HOUR_FX] > 292 && G.H[HOUR_FX] < 324, Math.round(G.H[HOUR_PX]));
    ok('hour shoulders ~360/600', isHourShoulderX(HOUR_L_SH) && isHourShoulderX(HOUR_R_SH), Math.round(G.H[HOUR_L_SH]) + '/' + Math.round(G.H[HOUR_R_SH]));
    ok('hour shoulders y~230', G.H[HOUR_L_SH] > 214 && G.H[HOUR_L_SH] < 248 && G.H[HOUR_R_SH] > 214 && G.H[HOUR_R_SH] < 248, Math.round(G.H[HOUR_L_SH]));
    ok('hour waist floor y~400', isHourWaistX(HOUR_CX) && G.H[HOUR_CX] > 384 && G.H[HOUR_CX] < 416, Math.round(G.H[HOUR_CX]));
    ok('hour waist ~90', isHourWaistX(HOUR_CX) && isHourWaistX(HOUR_CX - 40) && isHourWaistX(HOUR_CX + 40) && !isHourWaistX(HOUR_CX - 60) && HOUR_WAIST_W === 90);
    ok('hour no void', !isDeathVoid(HOUR_PX) && !isDeathVoid(HOUR_FX) && !isDeathVoid(HOUR_CX) && !isDeathVoid(HOUR_L_SH));
    ok('hour nums', HOUR_WALK === 0.85 && HOUR_CRATER === 0.75 && HOUR_STORM_P === 0.30 && GRAV === 260 && VK === 420);
    G.p = { x: HOUR_PX, y: G.H[HOUR_PX] - 14, r: 14, hp: 100, max: 100, side: 'p', id: 'p' };
    G.f = { x: HOUR_FX, y: G.H[HOUR_FX] - 14, r: 14, hp: 100, max: 100, side: 'f', id: 'f', ang: 115 };
    G.p2 = null; G.f2 = null;
    const hpx = spawnX('hour', 'p');
    const hpy = G.H[hpx | 0] - UNIT_R;
    const hth30 = 30 * Math.PI / 180;
    const hs30 = traceShot(hpx + Math.cos(hth30) * 18, hpy - 4 - Math.sin(hth30) * 18, 30, 94, 0, WEPS[0], G.H, G.p);
    ok('hour 30 thread far shore', isHourBank(hs30.x) && hs30.x > HOUR_R0 && !hs30.air, Math.round(hs30.x) + ',' + Math.round(hs30.y));
    const hLip = HOUR_LIP_L;
    const hley = G.H[hLip | 0] - UNIT_R;
    const h90 = 88;
    const hth90 = h90 * Math.PI / 180;
    const hs90 = traceShot(hLip + Math.cos(hth90) * 18, hley - 4 - Math.sin(hth90) * 18, h90, 95, 0, WEPS[0], G.H, G.p);
    ok('hour 90 dunk waist', isHourWaistX(hs90.x) && !hs90.air, Math.round(hs90.x) + ' a' + h90);
    const hth65 = 65 * Math.PI / 180;
    const hs65 = traceShot(hpx + Math.cos(hth65) * 18, hpy - 4 - Math.sin(hth65) * 18, 65, 32, 0, WEPS[0], G.H, G.p);
    ok('hour 65 clip near shoulder', isHourShoulderX(hs65.x) && hs65.x < HOUR_CX && !hs65.air, Math.round(hs65.x) + ',' + Math.round(hs65.y));
    const hWaist = { x: HOUR_CX, y: G.H[HOUR_CX] - 14, r: 14, hp: 100, max: 100, side: 'p', id: 'p' };
    const hSh = { x: HOUR_L_SH, y: G.H[HOUR_L_SH] - 14, r: 14, hp: 100, max: 100, side: 'f', id: 'f' };
    const hBank = { x: HOUR_PX, y: G.H[HOUR_PX] - 14, r: 14, hp: 100, max: 100, side: 'p', id: 'p' };
    ok('hour waist walk 0.85', inHourWaist(hWaist) && Math.abs(walkSpd(hWaist) - 90 * HOUR_WALK) < 0.01, walkSpd(hWaist));
    ok('hour shoulder walk full', isHourShoulderX(hSh.x) && Math.abs(walkSpd(hSh) - 78) < 0.01, walkSpd(hSh));
    ok('hour shore walk full', isHourBank(hBank.x) && Math.abs(walkSpd(hBank) - 90) < 0.01, walkSpd(hBank));
    G.storm = true;
    ok('hour waist no storm extra', inHourWaist(hWaist) && !onGrass(hWaist) && Math.abs(walkSpd(hWaist) - 90 * HOUR_WALK) < 0.01, walkSpd(hWaist));
    ok('hour shore wet grass', onGrass(hBank) && Math.abs(walkSpd(hBank) - 90 * STORM_WALK) < 0.01, walkSpd(hBank));
    G.storm = false;
    const hpSoakH = hWaist.hp;
    tickMoonWater(hWaist);
    tickCliffWater(hWaist);
    tickWellWater(hWaist);
    ok('hour no water soak', hWaist.hp === hpSoakH && inMoonWater(hWaist) === false && inWellWater(hWaist) === false, hWaist.hp);
    const hourDirtH = new Float32Array(VW);
    for (let i = 0; i < VW; i++) hourDirtH[i] = 400;
    G.H = hourDirtH;
    G.mapId = 'plain';
    carve(500, 400, 30);
    const hourDirt = G.H[500] - 400;
    G.mapId = 'hour';
    for (let i = 0; i < VW; i++) G.H[i] = 400;
    G.H[HOUR_L_SH] = HOUR_SHOULDER_Y;
    carve(HOUR_L_SH, HOUR_SHOULDER_Y, hourR(30, HOUR_L_SH));
    const shEat = G.H[HOUR_L_SH] - HOUR_SHOULDER_Y;
    ok('hour shoulder crater ~0.75', shEat > 4 && shEat < hourDirt * 0.92 && Math.abs(shEat / hourDirt - HOUR_CRATER) < 0.12, Math.round(shEat) + '/' + Math.round(hourDirt));
    G.H = buildHeight('hour');
    G.mapId = 'hour';
    const hourShoreY = G.H[HOUR_PX];
    carve(HOUR_PX, hourShoreY, 30);
    const hourShoreEat = G.H[HOUR_PX] - hourShoreY;
    ok('hour shore crater normal', Math.abs(hourShoreEat - hourDirt) < 3, Math.round(hourShoreEat) + '/' + Math.round(hourDirt));
    const heSh = Math.round(hourR(WEPS[1].crater, HOUR_L_SH));
    ok('hour shoulder 余震', wantQuake(WEPS[1], heSh) === true && heSh >= QUAKE_R * 0.65, heSh);
    G.H = buildHeight('hour');
    G.mapId = 'hour';
    G.kind = 'hall';
    G.wind = 0;
    G.ai = 1;
    G.p = { x: HOUR_CX, y: G.H[HOUR_CX] - 14, r: 14, hp: 100, max: 100, side: 'p', id: 'p' };
    G.f = { x: HOUR_FX, y: G.H[HOUR_FX] - 14, r: 14, hp: 100, max: 100, side: 'f', id: 'f', ang: 115 };
    G.p2 = null; G.f2 = null;
    ok('hour AI waist 高爆', inHourWaist(G.p) && pickAIWeapon(G.f) === 1);
    const himpV = { x: HOUR_CX, y: G.H[HOUR_CX], t: 1, hit: null };
    const hsc90 = scoreOne(himpV, WEPS[1], G.f, G.p, 88);
    const hsc30v = scoreOne(himpV, WEPS[1], G.f, G.p, 30);
    ok('hour AI prefer 90 dunk', hsc90 > hsc30v + 400, Math.round(hsc90) + '>' + Math.round(hsc30v));
    G.H = buildHeight('hour');
    G.mapId = 'hour';
    G.p = { x: HOUR_PX, y: G.H[HOUR_PX] - 14, r: 14, hp: 100, max: 100, side: 'p', id: 'p' };
    G.f = { x: HOUR_FX, y: G.H[HOUR_FX] - 14, r: 14, hp: 100, max: 100, side: 'f', id: 'f', ang: 115 };
    ok('hour AI far shore 普通', hourFarShore(G.f, G.p) && pickAIWeapon(G.f) === 0);
    const himpFar = { x: HOUR_PX, y: G.H[HOUR_PX], t: 1, hit: null };
    const hscSkip = scoreOne(himpFar, WEPS[0], G.f, G.p, 30);
    const hscHi = scoreOne(himpFar, WEPS[0], G.f, G.p, 65);
    ok('hour AI prefer 30 thread', hscSkip > hscHi + 400, Math.round(hscSkip) + '>' + Math.round(hscHi));
    G.p = { x: HOUR_LIP_L, y: G.H[HOUR_LIP_L] - 14, r: 14, hp: 50, max: 100, side: 'p', id: 'p' };
    G.turns = 3;
    ok('hour AI far lip 迟雷', (G.f.x < HOUR_CX) !== (G.p.x < HOUR_CX) && isHourLipX(G.p.x) && wantChiLei(G.f, G.p) === true && pickAIWeapon(G.f) === 7);
    G.kind = 'duo';
    const hdx0 = spawnAt('p', 0);
    const hdx1 = spawnAt('p', 1);
    const hdr0 = spawnAt('f', 0);
    const hdr1 = spawnAt('f', 1);
    ok('hour duo extras shore', isHourBank(hdx0) && isHourBank(hdx1) && isHourBank(hdr0) && isHourBank(hdr1), hdx1 + '/' + hdr1);
    ok('hour duo tops', hdx0 === HOUR_PX && hdr0 === HOUR_FX);
    ok('hour duo extras along', Math.abs(hdx1 - hdx0) >= 20 && Math.abs(hdr1 - hdr0) >= 20, Math.round(Math.abs(hdx1 - hdx0)));
    ok('hour duo not shoulder', !isHourShoulderX(hdx0) && !isHourShoulderX(hdx1) && !isHourShoulderX(hdr0) && !isHourShoulderX(hdr1));
    ok('hour duo not waist', !isHourWaistX(hdx1) && !isHourWaistX(hdr1));
    ok('hour duo not void', !isDeathVoid(hdx0) && !isDeathVoid(hdx1) && !isDeathVoid(hdr0) && !isDeathVoid(hdr1));
    G.kind = 'hall';
    ok('crate on hour shore', crateGroundOk(HOUR_PX) === true && crateGroundOk(HOUR_CX) === false && crateGroundOk(HOUR_L_SH) === false);
    ok('hour storm ~30', !stormForced('hour') && !stormBanned('hour') && HOUR_STORM_P === 0.30 && STORM_P === 0.35);
    ok('砂漏 name locked', MAP_NAME.hour === '砂漏' && HOUR_NAME === '砂漏' && MAP_IDS[22] === 'hour');
    ok('no banned hour', MAP_NAME.hour.indexOf('传送') < 0 && MAP_NAME.hour.indexOf('飞行') < 0 && MAP_NAME.hour.indexOf('三叉戟') < 0 && MAP_NAME.hour.indexOf('激怒') < 0 && MAP_NAME.hour.indexOf('天使') < 0 && MAP_NAME.hour.indexOf('恶魔') < 0 && MAP_NAME.hour.indexOf('风谷') < 0 && MAP_NAME.hour.indexOf('环坑') < 0);
    ok('maps 23 with 砂漏', MAP_IDS.length === 24 && MAP_NAME.hour === '砂漏' && MAP_NAME.ring === '环坑' && MAP_NAME.teeth === '齿岸' && MAP_NAME.cave === '洞顶' && MAP_NAME.well === '井口' && MAP_NAME.moon === '月池');
    ok('环坑 still after 砂漏', MAP_NAME.ring === '环坑' && MAP_IDS[21] === 'ring');
    ok('齿岸 still after 砂漏', MAP_NAME.teeth === '齿岸' && MAP_IDS[20] === 'teeth');
    ok('洞顶 still after 砂漏', MAP_NAME.cave === '洞顶' && MAP_IDS[19] === 'cave');
    ok('井口 still after 砂漏', MAP_NAME.well === '井口' && MAP_IDS[18] === 'well');
    ok('月池 still after 砂漏', MAP_NAME.moon === '月池');
    ok('叠珠 still 7 after 砂漏', WEPS[6] && WEPS[6].name === '叠珠' && WEPS[6].id === 7);
    ok('迟雷 still 8 after 砂漏', WEPS[7] && WEPS[7].name === '迟雷' && WEPS[7].id === 8);
    G.mapId = 'hour';
    G.H = buildHeight('hour');
    G.cave = null;
    falls.length = 0;
    dusts.length = 0;
    ok('砂漏 skip 坑尘', wantDust() === false && spawnDustBurst(HOUR_CX, G.H[HOUR_CX], 8) === 0 && dusts.length === 0);
    ok('落顶 still after 砂漏', FALL_NAME === '落顶' && wantFallChips() === false);
    ok('坑尘 still ring-only', DUST_NAME === '坑尘' && wantDust() === (!REDUCE && G.mapId === 'ring'));
    G.mapId = 'ring';
    G.H = buildHeight('ring');
    ok('坑尘 still ring after 砂漏', wantDust() === !REDUCE);
    G.mapId = 'hour';
    G.H = buildHeight('hour');
    ok('金匣袋 still after 砂漏', GOLD_BAG_NAME === '金匣袋' && GOLD_BAG_P === 0.40 && maybeBagCrate('gold') === 'gold' && BAG_CRATE_P === 0.50);
    ok('×3 fork still after 砂漏', bagForkAngles(65).join(',') === '57,65,73' && bagExtraCount(x3u) === 0);
    fireBagU({ x3: true, ang: 65, power: 70, wep: 0 });
    ok('×3 3 shells still after 砂漏', G.shots && G.shots.length === 3 && (!G.queue || G.queue.length === 0));
    fireBagU({ x2: true, ang: 65, power: 70, wep: 0, stam: 0 });
    ok('0 stam still after 砂漏', G.shots && G.shots.length === 1 && (!G.queue || G.queue.length === 0));
    ok('袋火 still after 砂漏', BAG_FIRE_NAME === '袋火' && BAG_TINT.x3 === '#dc143c');
    ok('堂袋 hud still after 砂漏', bagStackReadout(x2p5) === '2发×1.35' && bagStackReadout(x3u) === '3发×0.60');
    ok('余震 still after 砂漏', QUAKE_NAME === '余震' && HIT_STOP_DIRECT === 0.14);
    ok('雷泽 still after 砂漏', STORM_NAME === '雷泽' && stormForced('vale') && stormBanned('forge'));
    ok('蓄条 still after 砂漏', CHARGE_BAR_NAME === '蓄条' && CHARGE_T === 2);
    ok('ghost K still after 砂漏', G.ghostOn !== false && OPS.indexOf('K 残影') >= 0);
    ok('时尽 still after 砂漏', TURN_T === 18 && TURN_T_CORE === 14 && TURN_T_SUDDEN === 11);
    ok('no 9th wep after 砂漏', WEPS.length === 8 && WEPS[6].name === '叠珠' && WEPS[7].name === '迟雷');
    ok('CHARGE_T still 2 after 砂漏', CHARGE_T === 2 && TAP_POW === 12);
    ok('g vk v414', GRAV === 260 && VK === 420 && WIND_K === 2.05);
    ok('plus still additive after 砂漏', Math.abs(bagPlusMul(plusU) - 2.1) < 1e-9);
    ...
    ok('maps 23 after 腰沙', MAP_IDS.length === 24 && MAP_NAME.hour === '砂漏' && MAP_IDS[22] === 'hour' && MAP_NAME.ring === '环坑');
    ...
    G.mines = [];
    G.walk = WALK_PX;
    ok('ridge spawn', spawnX('ridge', 'p') === RIDGE_PX && spawnX('ridge', 'f') === RIDGE_FX);
    ok('ridge spawn on shores', isRidgeBank(RIDGE_PX) && isRidgeBank(RIDGE_FX), Math.round(G.H[RIDGE_PX]) + '/' + Math.round(G.H[RIDGE_FX]));
    ok('ridge shores y~308', G.H[RIDGE_PX] > 292 && G.H[RIDGE_PX] < 324 && G.H[RIDGE_FX] > 292 && G.H[RIDGE_FX] < 324, Math.round(G.H[RIDGE_PX]));
    ok('ridge peak ~480', isRidgeCrestX(RIDGE_CX) && G.H[RIDGE_CX] > 184 && G.H[RIDGE_CX] < 216, Math.round(G.H[RIDGE_CX]));
    ok('ridge peak y~200', G.H[RIDGE_CX] > 184 && G.H[RIDGE_CX] < 216, Math.round(G.H[RIDGE_CX]));
    ok('ridge base ~120', isRidgeCrestX(RIDGE_CX) && isRidgeCrestX(RIDGE_CX - 50) && isRidgeCrestX(RIDGE_CX + 50) && !isRidgeCrestX(RIDGE_CX - 72) && RIDGE_BASE_W === 120);
    ok('ridge flanks dirt', isRidgeFlankX(RIDGE_CX - 90) && isRidgeFlankX(RIDGE_CX + 90) && !isRidgeCrestX(RIDGE_CX - 90));
    ok('ridge no void', !isDeathVoid(RIDGE_PX) && !isDeathVoid(RIDGE_FX) && !isDeathVoid(RIDGE_CX) && !isDeathVoid(RIDGE_CX - 90));
    ok('ridge nums', RIDGE_WALK === 0.92 && RIDGE_CRATER === 0.70 && RIDGE_STORM_P === 0.30 && GRAV === 260 && VK === 420);
    G.p = { x: RIDGE_PX, y: G.H[RIDGE_PX] - 14, r: 14, hp: 100, max: 100, side: 'p', id: 'p' };
    G.f = { x: RIDGE_FX, y: G.H[RIDGE_FX] - 14, r: 14, hp: 100, max: 100, side: 'f', id: 'f', ang: 115 };
    G.p2 = null; G.f2 = null;
    const rgpx = spawnX('ridge', 'p');
    const rgpy = G.H[rgpx | 0] - UNIT_R;
    const rgth30 = 30 * Math.PI / 180;
    const rgs30 = traceShot(rgpx + Math.cos(rgth30) * 18, rgpy - 4 - Math.sin(rgth30) * 18, 30, 94, 0, WEPS[0], G.H, G.p);
    ok('ridge 30 skip far shore', isRidgeBank(rgs30.x) && rgs30.x > RIDGE_R0 && !rgs30.air, Math.round(rgs30.x) + ',' + Math.round(rgs30.y));
    const rgLip = RIDGE_LIP_L;
    const rgley = G.H[rgLip | 0] - UNIT_R;
    const rg90 = 86;
    const rgth90 = rg90 * Math.PI / 180;
    const rgs90 = traceShot(rgLip + Math.cos(rgth90) * 18, rgley - 4 - Math.sin(rgth90) * 18, rg90, 95, 0, WEPS[0], G.H, G.p);
    ok('ridge 90 dunk near flank', isRidgeFlankX(rgs90.x) && !rgs90.air, Math.round(rgs90.x) + ' a' + rg90);
    const rgth65 = 65 * Math.PI / 180;
    const rgs65 = traceShot(rgpx + Math.cos(rgth65) * 18, rgpy - 4 - Math.sin(rgth65) * 18, 65, 32, 0, WEPS[0], G.H, G.p);
    ok('ridge 65 clip near flank/crest', (isRidgeFlankX(rgs65.x) || isRidgeCrestX(rgs65.x)) && rgs65.x < RIDGE_CX && !rgs65.air, Math.round(rgs65.x) + ',' + Math.round(rgs65.y));
    const rgFlank = { x: RIDGE_CX - 90, y: G.H[RIDGE_CX - 90] - 14, r: 14, hp: 100, max: 100, side: 'p', id: 'p' };
    const rgCrest = { x: RIDGE_CX, y: G.H[RIDGE_CX] - 14, r: 14, hp: 100, max: 100, side: 'f', id: 'f' };
    const rgBank = { x: RIDGE_PX, y: G.H[RIDGE_PX] - 14, r: 14, hp: 100, max: 100, side: 'p', id: 'p' };
    ok('ridge flank walk 0.92', inRidgeFlank(rgFlank) && Math.abs(walkSpd(rgFlank) - 90 * RIDGE_WALK) < 0.01, walkSpd(rgFlank));
    ok('ridge crest walk full', isRidgeCrestX(rgCrest.x) && Math.abs(walkSpd(rgCrest) - 78) < 0.01, walkSpd(rgCrest));
    ok('ridge shore walk full', isRidgeBank(rgBank.x) && Math.abs(walkSpd(rgBank) - 90) < 0.01, walkSpd(rgBank));
    G.storm = true;
    ok('ridge flank no storm extra', inRidgeFlank(rgFlank) && !onGrass(rgFlank) && Math.abs(walkSpd(rgFlank) - 90 * RIDGE_WALK) < 0.01, walkSpd(rgFlank));
    ok('ridge shore wet grass', onGrass(rgBank) && Math.abs(walkSpd(rgBank) - 90 * STORM_WALK) < 0.01, walkSpd(rgBank));
    G.storm = false;
    const hpSoakR = rgFlank.hp;
    tickMoonWater(rgFlank);
    tickCliffWater(rgFlank);
    tickWellWater(rgFlank);
    ok('ridge no water soak', rgFlank.hp === hpSoakR && inMoonWater(rgFlank) === false && inWellWater(rgFlank) === false, rgFlank.hp);
    const ridgeDirtH = new Float32Array(VW);
    for (let i = 0; i < VW; i++) ridgeDirtH[i] = 400;
    G.H = ridgeDirtH;
    G.mapId = 'plain';
    carve(500, 400, 30);
    const ridgeDirt = G.H[500] - 400;
    G.mapId = 'ridge';
    for (let i = 0; i < VW; i++) G.H[i] = 400;
    G.H[RIDGE_CX] = RIDGE_PEAK_Y;
    carve(RIDGE_CX, RIDGE_PEAK_Y, ridgeR(30, RIDGE_CX));
    const crestEat = G.H[RIDGE_CX] - RIDGE_PEAK_Y;
    ok('ridge crest crater ~0.70', crestEat > 4 && crestEat < ridgeDirt * 0.92 && Math.abs(crestEat / ridgeDirt - RIDGE_CRATER) < 0.12, Math.round(crestEat) + '/' + Math.round(ridgeDirt));
    G.H = buildHeight('ridge');
    G.mapId = 'ridge';
    const ridgeShoreY = G.H[RIDGE_PX];
    carve(RIDGE_PX, ridgeShoreY, 30);
    const ridgeShoreEat = G.H[RIDGE_PX] - ridgeShoreY;
    ok('ridge shore crater normal', Math.abs(ridgeShoreEat - ridgeDirt) < 3, Math.round(ridgeShoreEat) + '/' + Math.round(ridgeDirt));
    const heCrest = Math.round(ridgeR(WEPS[1].crater, RIDGE_CX));
    ok('ridge crest 余震', wantQuake(WEPS[1], heCrest) === true && heCrest >= QUAKE_R * 0.65, heCrest);
    G.H = buildHeight('ridge');
    G.mapId = 'ridge';
    G.kind = 'hall';
    G.wind = 0;
    G.ai = 1;
    G.p = { x: RIDGE_CX - 90, y: G.H[RIDGE_CX - 90] - 14, r: 14, hp: 100, max: 100, side: 'p', id: 'p' };
    G.f = { x: RIDGE_FX, y: G.H[RIDGE_FX] - 14, r: 14, hp: 100, max: 100, side: 'f', id: 'f', ang: 115 };
    G.p2 = null; G.f2 = null;
    ok('ridge AI flank 高爆', inRidgeFlank(G.p) && pickAIWeapon(G.f) === 1);
    const rgimpV = { x: RIDGE_CX - 90, y: G.H[RIDGE_CX - 90], t: 1, hit: null };
    const rgsc90 = scoreOne(rgimpV, WEPS[1], G.f, G.p, 88);
    const rgsc30v = scoreOne(rgimpV, WEPS[1], G.f, G.p, 30);
    ok('ridge AI prefer 90 dunk', rgsc90 > rgsc30v + 400, Math.round(rgsc90) + '>' + Math.round(rgsc30v));
    G.H = buildHeight('ridge');
    G.mapId = 'ridge';
    G.p = { x: RIDGE_PX, y: G.H[RIDGE_PX] - 14, r: 14, hp: 100, max: 100, side: 'p', id: 'p' };
    G.f = { x: RIDGE_FX, y: G.H[RIDGE_FX] - 14, r: 14, hp: 100, max: 100, side: 'f', id: 'f', ang: 115 };
    ok('ridge AI far shore 普通', ridgeFarShore(G.f, G.p) && pickAIWeapon(G.f) === 0);
    const rgimpFar = { x: RIDGE_PX, y: G.H[RIDGE_PX], t: 1, hit: null };
    const rgscSkip = scoreOne(rgimpFar, WEPS[0], G.f, G.p, 30);
    const rgscHi = scoreOne(rgimpFar, WEPS[0], G.f, G.p, 65);
    ok('ridge AI prefer 30 skip', rgscSkip > rgscHi + 400, Math.round(rgscSkip) + '>' + Math.round(rgscHi));
    G.p = { x: RIDGE_LIP_L, y: G.H[RIDGE_LIP_L] - 14, r: 14, hp: 50, max: 100, side: 'p', id: 'p' };
    G.turns = 3;
    ok('ridge AI far lip 迟雷', (G.f.x < RIDGE_CX) !== (G.p.x < RIDGE_CX) && isRidgeLipX(G.p.x) && wantChiLei(G.f, G.p) === true && pickAIWeapon(G.f) === 7);
    G.kind = 'duo';
    const rgdx0 = spawnAt('p', 0);
    const rgdx1 = spawnAt('p', 1);
    const rgdr0 = spawnAt('f', 0);
    const rgdr1 = spawnAt('f', 1);
    ok('ridge duo extras shore', isRidgeBank(rgdx0) && isRidgeBank(rgdx1) && isRidgeBank(rgdr0) && isRidgeBank(rgdr1), rgdx1 + '/' + rgdr1);
    ok('ridge duo tops', rgdx0 === RIDGE_PX && rgdr0 === RIDGE_FX);
    ok('ridge duo extras along', Math.abs(rgdx1 - rgdx0) >= 20 && Math.abs(rgdr1 - rgdr0) >= 20, Math.round(Math.abs(rgdx1 - rgdx0)));
    ok('ridge duo not crest', !isRidgeCrestX(rgdx0) && !isRidgeCrestX(rgdx1) && !isRidgeCrestX(rgdr0) && !isRidgeCrestX(rgdr1));
    ok('ridge duo not flank', !isRidgeFlankX(rgdx1) && !isRidgeFlankX(rgdr1));
    ok('ridge duo not void', !isDeathVoid(rgdx0) && !isDeathVoid(rgdx1) && !isDeathVoid(rgdr0) && !isDeathVoid(rgdr1));
    G.kind = 'hall';
    ok('crate on ridge shore', crateGroundOk(RIDGE_PX) === true && crateGroundOk(RIDGE_CX) === false && crateGroundOk(RIDGE_CX - 90) === false);
    ok('ridge storm ~30', !stormForced('ridge') && !stormBanned('ridge') && RIDGE_STORM_P === 0.30 && STORM_P === 0.35);
    ok('折脊 name locked', MAP_NAME.ridge === '折脊' && RIDGE_NAME === '折脊' && MAP_IDS[23] === 'ridge');
    ok('no banned ridge', MAP_NAME.ridge.indexOf('传送') < 0 && MAP_NAME.ridge.indexOf('飞行') < 0 && MAP_NAME.ridge.indexOf('三叉戟') < 0 && MAP_NAME.ridge.indexOf('激怒') < 0 && MAP_NAME.ridge.indexOf('天使') < 0 && MAP_NAME.ridge.indexOf('恶魔') < 0 && MAP_NAME.ridge.indexOf('砂漏') < 0 && MAP_NAME.ridge.indexOf('齿岸') < 0 && MAP_NAME.ridge.indexOf('断崖') < 0);
    ok('maps 24 with 折脊', MAP_IDS.length === 24 && MAP_NAME.ridge === '折脊' && MAP_NAME.hour === '砂漏' && MAP_NAME.ring === '环坑' && MAP_NAME.teeth === '齿岸' && MAP_NAME.cave === '洞顶' && MAP_NAME.well === '井口' && MAP_NAME.moon === '月池');
    ok('砂漏 still after 折脊', MAP_NAME.hour === '砂漏' && MAP_IDS[22] === 'hour');
    ok('环坑 still after 折脊', MAP_NAME.ring === '环坑' && MAP_IDS[21] === 'ring');
    ok('齿岸 still after 折脊', MAP_NAME.teeth === '齿岸' && MAP_IDS[20] === 'teeth');
    ok('洞顶 still after 折脊', MAP_NAME.cave === '洞顶' && MAP_IDS[19] === 'cave');
    ok('井口 still after 折脊', MAP_NAME.well === '井口' && MAP_IDS[18] === 'well');
    ok('月池 still after 折脊', MAP_NAME.moon === '月池');
    ok('叠珠 still 7 after 折脊', WEPS[6] && WEPS[6].name === '叠珠' && WEPS[6].id === 7);
    ok('迟雷 still 8 after 折脊', WEPS[7] && WEPS[7].name === '迟雷' && WEPS[7].id === 8);
    G.mapId = 'ridge';
    G.H = buildHeight('ridge');
    G.cave = null;
    falls.length = 0;
    ...
    G.mapId = 'ring';
    G.H = buildHeight('ring');
    ok('坑尘 still ring after 折脊', wantDust() === !REDUCE);
    G.mapId = 'hour';
    G.H = buildHeight('hour');
    ok('腰沙 still hour after 折脊', wantSand() === !REDUCE);
    G.mapId = 'ridge';
    G.H = buildHeight('ridge');
    ok('金匣袋 still after 折脊', GOLD_BAG_NAME === '金匣袋' && GOLD_BAG_P === 0.40 && maybeBagCrate('gold') === 'gold' && BAG_CRATE_P === 0.50);
    ok('×3 fork still after 折脊', bagForkAngles(65).join(',') === '57,65,73' && bagExtraCount(x3u) === 0);
    fireBagU({ x3: true, ang: 65, power: 70, wep: 0 });
    ok('×3 3 shells still after 折脊', G.shots && G.shots.length === 3 && (!G.queue || G.queue.length === 0));
    fireBagU({ x2: true, ang: 65, power: 70, wep: 0, stam: 0 });
    ok('0 stam still after 折脊', G.shots && G.shots.length === 1 && (!G.queue || G.queue.length === 0));
    ok('袋火 still after 折脊', BAG_FIRE_NAME === '袋火' && BAG_TINT.x3 === '#dc143c');
    ok('堂袋 hud still after 折脊', bagStackReadout(x2p5) === '2发×1.35' && bagStackReadout(x3u) === '3发×0.60');
    ok('余震 still after 折脊', QUAKE_NAME === '余震' && HIT_STOP_DIRECT === 0.14);
    ok('雷泽 still after 折脊', STORM_NAME === '雷泽' && stormForced('vale') && stormBanned('forge'));
    ok('蓄条 still after 折脊', CHARGE_BAR_NAME === '蓄条' && CHARGE_T === 2);
    ok('腰沙 still hour-only', SAND_NAME === '腰沙' && wantSand() === false);
    ok('ghost K still after 折脊', G.ghostOn !== false && OPS.indexOf('K 残影') >= 0);
    ok('时尽 still after 折脊', TURN_T === 18 && TURN_T_CORE === 14 && TURN_T_SUDDEN === 11);
    ok('no 9th wep after 折脊', WEPS.length === 8 && WEPS[6].name === '叠珠' && WEPS[7].name === '迟雷');
    ok('CHARGE_T still 2 after 折脊', CHARGE_T === 2 && TAP_POW === 12);
    ok('g vk v416', GRAV === 260 && VK === 420 && WIND_K === 2.05);
    ok('plus still additive after 折脊', Math.abs(bagPlusMul(plusU) - 2.1) < 1e-9);
